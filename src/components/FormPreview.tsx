        ...
        <div className="space-y-4">
          {questions.map((q, idx) => (
            <div key={q.id} className="bg-white rounded-2xl shadow-sm border border-sky-100 p-6">
              ...
              <div className="ml-9">
                {q.type === "short" && <input className="w-full px-4 py-3 border rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} placeholder="คำตอบของคุณ" />}
                {q.type === "long" && <textarea className="w-full px-4 py-3 border rounded-lg h-32" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} placeholder="คำตอบของคุณ" />}
                ...
                {q.type === "dropdown" && <select className="w-full px-4 py-3 border rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))}><option value="">เลือก...</option>{q.options.map((opt) => <option key={opt} value={opt}>{opt}</option>)}</select>}
                {q.type === "date" && <input type="date" className="w-full px-4 py-3 border rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} />}
                {q.type === "time" && <input type="time" className="w-full px-4 py-3 border rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} />}
              </div>
            </div>
          ))}
        </div>
