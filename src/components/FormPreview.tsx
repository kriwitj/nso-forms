...
export default function FormPreview({ formId }: { formId: string }) {
  ...
  return (
    <div className="min-h-screen py-6">
      <main className="max-w-3xl mx-auto px-4">
        <div className="bg-white rounded-2xl shadow-sm border border-sky-100 border-t-8 border-t-sky-600 mb-4 overflow-hidden p-6">
        ...
        <div className="space-y-4">
          {questions.map((q, idx) => (
            <div key={q.id} className="bg-white rounded-2xl shadow-sm border border-sky-100 p-6">
              ...
              <div className="ml-9">
                {q.type === "short" && <input className="w-full px-4 py-3 border border-sky-200 rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} placeholder="คำตอบของคุณ" />}
                {q.type === "long" && <textarea className="w-full px-4 py-3 border border-sky-200 rounded-lg h-32" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} placeholder="คำตอบของคุณ" />}
                {(q.type === "multiple" || q.type === "scale") && (q.type === "scale" ? ["1", "2", "3", "4", "5"] : q.options).map((opt) => <label key={opt} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer"><input type="radio" name={`q_${q.id}`} value={opt} checked={(answers[q.id] ?? "") === opt} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} className="w-4 h-4 accent-sky-600" /><span>{opt}</span></label>)}
                {q.type === "checkbox" && q.options.map((opt) => {
                  const set = new Set((answers[q.id] ?? "").split(",").map((s) => s.trim()).filter(Boolean));
                  return <label key={opt} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer"><input type="checkbox" className="w-4 h-4 accent-sky-600" checked={set.has(opt)} onChange={(e) => {
                    const next = new Set(set); if (e.target.checked) next.add(opt); else next.delete(opt); setAnswers((p) => ({ ...p, [q.id]: Array.from(next).join(", ") }));
                  }} /><span>{opt}</span></label>;
                })}
                {q.type === "dropdown" && <select className="w-full px-4 py-3 border border-sky-200 rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))}><option value="">เลือก...</option>{q.options.map((opt) => <option key={opt} value={opt}>{opt}</option>)}</select>}
                {q.type === "date" && <input type="date" className="w-full px-4 py-3 border border-sky-200 rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} />}
                {q.type === "time" && <input type="time" className="w-full px-4 py-3 border border-sky-200 rounded-lg" value={answers[q.id] ?? ""} onChange={(e) => setAnswers((p) => ({ ...p, [q.id]: e.target.value }))} />}
              </div>
            </div>
          ))}
        </div>
